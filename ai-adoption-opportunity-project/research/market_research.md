# Market Research: AI Adoption in Logistics & Freight Forwarding

**Project:** AI Adoption Opportunity — Document Intelligence
**Client:** CEO Olaf Müller, Hamburg-based logistics and transportation company (500 employees, 16 locations, 8 countries)


---

## 1. Industry Overview

The global freight forwarding market is valued at USD 178 billion in 2025 and projected to reach
USD 250 billion by 2034, driven by a CAGR of 3.8%. AI and machine learning are already improving
route optimisation, predictive analytics, and automated document processing across the sector.
*(Source: GlobeNewswire / ResearchAndMarkets, June 2025 —
https://www.globenewswire.com/news-release/2025/06/24/3104015/28124/en/Freight-Forwarding-Industry-Outlook-Report-2025-2034-AI-and-Blockchain-Revolutionizing-the-249-Bn-Market.html)*

The AI-in-supply-chain market specifically is valued at USD 9.94 billion in 2025 and projected to
reach USD 236 billion by 2035 — a 30%+ CAGR.
*(Source: Precedence Research via Open Sky Group, April 2026 —
https://openskygroup.com/supply-chain-ai-statistics/)*

---

## 2. AI Adoption Signals

### 2.1 Industry-wide adoption rates

- **70%** of logistics and transportation companies now report adopting AI solutions — up 17% year-on-year.
  Top gains: fleet planning (36%), route optimisation (35%), operational efficiency (34%).
  *(Source: Penske Transportation Leaders Survey 2025 —
  https://www.truckinginfo.com/10241279/ai-in-logistics-penske-survey-uncovers-surging-adoption-rising-concerns)*

- **72%** of logistics employees adopted AI tools in 2024 — the highest rate across all industries.
  *(Source: ActivTrak 2025 via Open Sky Group —
  https://openskygroup.com/supply-chain-ai-statistics/)*

- **94%** of supply chain companies plan to use AI or generative AI for decision support within two years.
  *(Source: ABI Research 2025 via Open Sky Group —
  https://openskygroup.com/supply-chain-ai-statistics/)*

- **85%** of executives plan to increase AI spending in 2026, with one in five expecting a 20%+ increase.
  *(Source: Supply Chain Brain 2025 via Open Sky Group —
  https://openskygroup.com/supply-chain-ai-statistics/)*

### 2.2 Mid-market gap

Only **28%** of mid-sized logistics providers (USD 10M–100M annual revenue) have implemented
comprehensive AI solutions, compared to 73% of large enterprises. Cloud-based, consumption-priced
AI tools are beginning to close this gap.
*(Source: DocShipper 2025 —
https://docshipper.com/logistics/ai-changing-logistics-supply-chain-2025/)*

### 2.3 Generative AI signal

**29%** of supply chain executives plan to fundamentally reinvent their supply chain management
using generative AI — specifically in forecasting, capacity planning, and pricing optimisation.
*(Source: Accenture study via Maersk, December 2024 —
https://www.maersk.com/insights/digitalisation/2024/12/18/what-to-expect-in-2025-supply-chain-tech-trends)*

### 2.4 Company-specific adoption signals — Müller's company

While industry-wide adoption rates establish the broader context, the following operational characteristics of Müller's company are themselves classic indicators of AI readiness and fit — independently of the industry trend. Each signal maps directly to a condition where Document Intelligence delivers measurable value.

| Signal | Present | Why it matters for AI adoption |
|---|---|---|
| Existing TMS usage | ✓ | Confirms data infrastructure and operational discipline; Document Intelligence integrates as an additional layer, not a replacement |
| International operations (multi-country, multi-customs regime) | ✓ | 8 countries × multiple customs regimes = high daily document volume and regulatory complexity — the core conditions for automation ROI |
| E-commerce customers | ✓ | E-commerce logistics generates high-frequency, standardised document flows — well-suited to AI extraction and pre-population |
| Contract logistics | ✓ | Long-term client agreements require consistent, auditable documentation — AI-generated records directly support compliance and client reporting obligations |
| Hazardous goods logistics | ✓ | Hazmat shipments require precise, error-free documentation across multiple regulatory frameworks — the cost of a documentation error is disproportionately high, making accuracy automation especially valuable |

**Reading the signals:** All five indicators are present. This is not a company that needs to build readiness for AI — the operational environment that makes Document Intelligence valuable already exists. The question is not whether to adopt, but where to start.

---

## 3. ROI & Cost Savings Evidence

| Metric | Figure | Source |
|---|---|---|
| Average cost savings — AI adopters | 15–20% | Capgemini via Gitnux 2025 |
| Companies reporting ROI within 12 months | 70% | Capgemini via Gitnux 2025 |
| Logistics cost improvement — early adopters | 15% | StartUs Insights / McKinsey 2025 |
| Inventory level reduction — early adopters | 35% | StartUs Insights 2025 |
| Service level improvement — early adopters | 65% | StartUs Insights 2025 |
| Margin improvement for mid-market 3PL | 30–75% | European Logistics Association 2025 |
| Median ROI over 3 years | 3.5x investment | McKinsey via DocShipper 2025 |

*(Sources: https://gitnux.org/ai-in-the-supply-chain-industry-statistics/ /
https://www.startus-insights.com/innovators-guide/ai-in-supply-chain/ /
https://thinking.inc/en/industry-service/logistics-ai-roi/)*

---

## 4. Benchmark Companies

*Source types are labelled for each entry: [Company Statement], [Annual Report], [Trade Publication], [Case Study], [Press Release], [Company Blog]*

### Maersk — Large enterprise
- Confirmed "ample proof points" of generative AI in forecasting, capacity planning, and pricing.
  Market adaptation time reduced from months to weeks.
  *[Company Statement] Maersk SVP, December 2024 —
  https://www.maersk.com/insights/digitalisation/2024/12/18/what-to-expect-in-2025-supply-chain-tech-trends*
- AI-driven predictive maintenance reduced vessel downtime by 30%, saving USD 300M+ annually.
  *[Trade Publication] DocShipper 2025 — https://docshipper.com/logistics/ai-changing-logistics-supply-chain-2025/*
- AI warehouse robotics increased inventory pickup by 33% and tripled sorting speed in UK warehouses.
  *[Trade Publication] DisruptionHub — https://disruptionhub.com/how-does-maersk-use-ai/*
- Strategic goal: zero-touch logistics within 5–7 years.

### DB Schenker — Large enterprise
- Built internal "Forecasting Fortune Teller" AI generating ~6,300 forecast series per day.
  *[Company Blog] DB Schenker Insights, August 2025 —
  https://www.dbschenker.com/global/insights/blog/unveiling-the-forecasting-fortune-teller-1708852*
- Deployed AI-powered network simulation in Bulgaria — tested Black Friday peaks, hub changes,
  route modifications before committing capital.
  *[Trade Publication] Transmetrics.ai / DB Schenker blog —
  https://www.transmetrics.ai/blog/supply-chain-logistics-technology-trends/*
- Centralised AI platform built on Microsoft Azure — one model adaptable to all country-specific needs.
  *[Company Page] DB Schenker Global — https://www.dbschenker.com/global/about/innovation-digitalization/data-analytics-artificial-intelligence*

### Kuehne+Nagel — Large enterprise
- Customs division achieved standout growth in 2024 driven by AI-supported customs services.
  *[Annual Report] Kuehne+Nagel 2024 —
  https://2024-annual-report.kuehne-nagel.com/corporate-brochure/english/interview-with-the-ceo*
- Deployed AI document recognition on Road Carrier platform — automated POD validation and
  NLP driver feedback analysis across Europe, Africa, and Middle East.
  *[Press Release] Kuehne+Nagel Newsroom, October 2024 —
  https://newsroom.kuehne-nagel.com/kuehnenagel-continues-to-digitise-its-cooperation-with-road-carriers-powered-by-ai/*
- Demand for AI-supported customs services increased noticeably in 2025, contributing to record
  Contract Logistics results.
  *[Annual Results Statement] Kuehne+Nagel 2025 —
  https://newsroom.kuehne-nagel.com/kuehnenagel-reports-solid-earnings-in-2025/*

### Customs Support Group (CSG) — Mid-market structural comparator
CSG is the closest publicly documented structural benchmark to Müller's company: a multi-country
customs and trade solutions provider operating across 14 European markets, serving freight forwarders
of comparable scale and document complexity.

- Deployed AI-powered Customs SmartAssist using OCR, ML, and NLP to process documents in any
  format or quality — including handwritten notes, fax copies, and mobile phone photos.
  *[Press Release] CSG, May 2025 —
  https://www.customssupport.com/customs-support-group-deploys-ai-powered-smart-document-processing-across-europe/*
- Traffic light confidence scoring system: green (auto-approved), orange (human review), red
  (manual entry required) — same model recommended for Müller's implementation.
  *[Company Page] CSG AI detail, March 2026 — https://www.customssupport.com/artificial-intelligence-ai-meets-customs/*
- Results measured: 99% efficiency improvement for simple repetitive tasks; 36% for complex
  declarations; 4 million customs declarations processed digitally per year; Italy hit December
  targets two months early.
  *[Case Study / Operational Reporting] Intelligent CIO Europe, April 2025 —
  https://www.intelligentcio.com/eu/2025/04/11/customs-support-group-carries-industry-from-burden-to-business-opportunity-through-ai-innovation/*

**Why CSG matters for Müller:** Unlike Maersk or DB Schenker, CSG operates at a scale and
document complexity directly comparable to a 500-person, multi-corridor 3PL. Their results are
the most transferable benchmark in this research.

---

## 5. Market Risks & Barriers

| Barrier | Prevalence | Source |
|---|---|---|
| AI below expectations | 60% of companies that invested | RELEX Solutions State of Supply Chain 2025 |
| Budget constraints | 43% cite as primary barrier | RELEX Solutions 2025 |
| Poor data quality | 39% cite as primary barrier | RELEX Solutions 2025 |
| Talent shortage for AI implementation | 44% | Noloco 2025 |
| No formal AI strategy in place | 77% of supply chain organisations | Gartner 2025 |
| AI initiatives exceeding budget | 62% by average of 45% | Gartner via DocShipper 2025 |
| Workforce digital literacy gap | 68% warehouse operators affected | DHL Logistics Trend Radar 2025 |

*(Sources: https://www.logisticsmgmt.com/article/ai_in_supply_chain_software_opportunities_challenges_and_the_future_of_automation /
https://openskygroup.com/supply-chain-ai-statistics/ /
https://thinking.inc/en/industry-service/ai-in-logistics/)*

---

## 6. Regulatory Signals

### EU Customs Reform (2024–2025)
The most ambitious reform of EU customs rules since 1968 introduces the EU Customs Data Hub.
AI will be used by customs authorities to analyse data and predict problems before goods begin their
journey. A new "trust and check trader" tier rewards companies with clean, accurate, auditable customs
data with faster clearance and fewer inspections.
*(Source: European Commission —
https://taxation-customs.ec.europa.eu/customs/eu-customs-reform_en)*

### EU AI Act
Full enforcement from 2 August 2026. Document Intelligence for logistics is classified as
**minimal risk** — no conformity assessment required. 53.8% of German enterprises had implemented
no concrete compliance measures as of November 2025.
*(Source: Deloitte November 2025 via Sphere Partners —
https://www.sphereinc.com/blogs/eu-ai-act-risk-classification-guide)*

### CSRD
Corporate Sustainability Reporting Directive expands to mid-size companies from 2026–2027.
Shipment data extracted for customs purposes (mode, distance, weight, carrier) directly feeds
CSRD emissions reporting — eliminating a separate data collection process.

---

## 7. Key Sources Index

| Source | Type | URL |
|---|---|---|
| Penske Transportation Leaders Survey 2025 | Industry survey | https://www.truckinginfo.com/10241279/ai-in-logistics-penske-survey-uncovers-surging-adoption-rising-concerns |
| Open Sky Group Supply Chain AI Statistics 2026 | Aggregated research | https://openskygroup.com/supply-chain-ai-statistics/ |
| Maersk 2025 Supply Chain Tech Trends | Company statement | https://www.maersk.com/insights/digitalisation/2024/12/18/what-to-expect-in-2025-supply-chain-tech-trends |
| DB Schenker Data Analytics & AI | Company page | https://www.dbschenker.com/global/about/innovation-digitalization/data-analytics-artificial-intelligence |
| Kuehne+Nagel Annual Report 2024 | Annual report | https://2024-annual-report.kuehne-nagel.com/corporate-brochure/english/interview-with-the-ceo |
| The Thinking Company — AI in Logistics 2026 | Industry guide | https://thinking.inc/en/industry-service/ai-in-logistics/ |
| StartUs Insights — AI in Supply Chain | Industry guide | https://www.startus-insights.com/innovators-guide/ai-in-supply-chain/ |
| DocShipper — AI Changing Logistics 2025 | Trade publication | https://docshipper.com/logistics/ai-changing-logistics-supply-chain-2025/ |
| GlobeNewswire — Freight Forwarding Market 2025–2034 | Market report | https://www.globenewswire.com/news-release/2025/06/24/3104015/28124/en/Freight-Forwarding-Industry-Outlook-Report-2025-2034-AI-and-Blockchain-Revolutionizing-the-249-Bn-Market.html |
| European Commission EU Customs Reform | Government source | https://taxation-customs.ec.europa.eu/customs/eu-customs-reform_en |
